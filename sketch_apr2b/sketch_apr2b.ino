...
const char *host = "192.168.141.140"; // 服务器地址
const int port = 9050;              // 服务器端口

// 创建NTP和HTTP客户端对象
WiFiUDP ntpUDP;                                              // NTPClient库需要
NTPClient timeClient(ntpUDP, "192.168.141.140");               // NTP服务器地址
ESP32Time rtc(8 * 3600);                                     // 时区设置为东八区
HTTPClient http;                                             // HTTPClient库需要
const char *serverUrl = "http://192.168.141.140:8020/olddata"; // 不加http://连不上
